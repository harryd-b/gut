# Phase 105 — entry 6: the joint two-geodesic arena (JOINT-1) and the mirror lemma L-MIRROR (REFEREED 2026-07-26; CONFIRMED WITH REPAIRS; L-MIRROR PROVED — see Amendment)
...
*Context-free adversarial referee pass completed 2026-07-26; report verbatim in `reviews/REPORTS-phase105-entry6-referee-2026-07-26.md`. The referee ran independent numerics (different profiles, charges, ramp widths, grid; deviations ≤ 8×10⁻¹²) and verified Lemma A2's boundary dynamics numerically. Standing AI-referee caveat applies.*

### A.1 Overall ruling

**Entry 6 CONFIRMED WITH REPAIRS. Both consistency mandates PASS:** the cross-invariant D₁₂ = Q₁Q₂·î reappears internally in the joint arena (Box 2, mathematics fully verified including the full-real-σ recovery), and the diagonal rates survive by embedding (Box 3, verified including the Tomiyama caveat). **L-MIRROR is PROVED as registered** (Box 4; both the derivation's and the referee's independent numerics confirm every identity), which settles the conjugation entry's open lemma — Dconj's Box 4a law D₁₂(mirror-1, 2) = −D₁₂ now holds [refereed] in the E2 class. Repairs adopted below.

### A.2 Correction C-J1 — Box 1 re-scoped: the no-go is about UNIVERSAL arenas; new open gap registered

Lemma A2 is correct in full (dynamics monotone and numerically verified; GAP-2 closable by one-particle point-triviality of the quasi-free net — a distribution supported at a point has divergent H^{1/2} norm). But requirement (R2) — fiber contains the FULL leg algebras — is forced only for a *universal* arena (one hosting arbitrary defects on both geodesics; the defect generators then generate A(Iᵢ)). Box 1 is re-scoped accordingly:

> **[BOX 1, repaired] No UNIVERSAL joint arena for a crossing pair retains an interval-local fiber; the minimal honest universal arena is M_Λ = (B(H)⊗̄L∞(S¹))⋊Λ, in which the ledger survives as the exact center of the collapsed fiber.**

**New open gap (Q-FIB):** for a FIXED pair of defects, does the Λ-orbit of the two implementer lines (plus the E1 truncated-cocycle families) generate B(H), or is there a proper local fiber? Density of the Λ-orbit span {ηᵢ∘g⁻¹} is plausible (Schottky spreading) but unproven; no collapse theorem, no counterexample. Registered.

### A.3 Correction C-J2 — Box 2 deflated and charge-qualified

Mathematics confirmed: commutator-phase invariance under phase redefinitions, representative-independence within the E2 class, and the Moore–Mackey classification recovering the full real σ from the one-parameter lines. Two repairs: (i) the abelianness equivalence requires **Q₁Q₂ ≠ 0** (neutral profiles commute even across crossing axes); (ii) internality rhetoric deflated — every ingredient already lives in the vacuum representation; **the joint arena hosts the invariant, it does not create it**. The true theorem retained: for nonzero charges, the group generated by the two implementer lines is abelian ⟺ σ = 0 ⟺ the axes do not cross (the one-parameter formulation is what makes this an iff, dodging the e^{iσ} trivialization trap).

### A.4 Correction C-J3 — proper-outerness step supplied for §1.4

Z(M_Λ) = L∞(S¹)^Λ⊗1 requires proper outerness of each g ≠ e on the fiber, not argued in the derivation; the referee supplied it (an inner restriction would fix a positive-measure piece of the center pointwise — impossible for a Möbius map ≠ id). Schottky/second-kind center remark confirmed. Adopted.

### A.5 L-MIRROR — PROVED [refereed], with the residue stated

Sharp and smooth proofs verified; referee's independent numerics (quartic ramps, asymmetric widths, Q₁ = −1.5, Q₂ = 0.7) reproduce all four identities to ≤ 8×10⁻¹². The out-of-domain adjudication of the winding counterexamples is ruled LEGITIMATE, and strengthened by a **new referee finding: σ is not invariant under neutral in-leg redistribution of transport** (moving a ramp across the other axis's anchor changes σ from −Q₁Q₂ to 0) — so the plateau/anchor-ramp restriction is load-bearing, and the anchor placement is the unique canonical one determined by the geodesic's endpoints alone. Also refereed: the corollary (mirroring defect 2 across r₁ leaves σ unchanged) is forced by the E5 identities and composes correctly with the both-mirrored flip; its geometric reading (r₁ conjugates γ₂ to a geodesic crossing with the same î) is coherent, consistent with r₁γ₁r₁ = γ₁⁻¹.

**Residue, stated per the referee's order (registered as open question Q-WIND):** whether an intrinsic, convention-free mutual phase exists for genuine WINDING implementers (sectorial/cocycle-theoretic), and whether it agrees with the plateau value. The winding-implementer mirror question is reclassified as **ill-posed pending such a definition** — not answered. (Natural literature-search target: charged-sector/BMT intertwiner statistics.)

### A.6 Minor

The derivation's GAP numbering skips GAP-3 (cosmetic; noted, not renumbered — labels are load-bearing in cross-references). §1.1's universality assumption is now surfaced by C-J1.

### A.7 Dictionary status after this entry

Six refereed items standing: the five entries (D, D₁₂, D_spin, D_fus, Dconj — Dconj's Box 4a now restored via L-MIRROR) plus the joint-arena structure theorem (universal no-go + M_Λ construction + internal recovery of D₁₂ + diagonal survival). Open registry: JOIN-4a″ (completeness), Q-FIB (fixed-pair fiber collapse), Q-WIND (intrinsic winding mutual phase), C-F3 (branch–crossing sign correlation), KNOT-Q (geometry bridge), JOIN-4b/K-DATA (standing falsifier), nested/disjoint joint fibers (GAP-7). Next per the adopted plan: entry 7, the conjugacy-class upgrade D̄₁₂.
