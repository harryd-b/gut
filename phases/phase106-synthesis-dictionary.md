...
**B1 — JOIN-4a″ (completeness). [RESOLVED IN THE NEGATIVE, 2026-07-26 — AI-refereed; see phases/phase108-JOIN4a2-resolution.md.]** The question — is φ = ω∘α in the exact unitary orbit of ω on A(I)? — is answered NO: no local unitary (or isometry) fakes the defect state, with the sharp criterion orbit ⟺ ĥ ∈ L² ⟺ bounded cocycle norm (on the smooth BMT class, ⟺ Q = 0). D quantifies the obstruction. Consequences: the dictionary is complete in the strong sense (the defect is a state-level fact, not relational bookkeeping); permanence is upgraded to a theorem (exact local erasure impossible); and (ω, ω∘α) is an explicit witness pair in the Haagerup–Størmer/Isono circle — the program's headline theorem and its TOP PRIORITY for human verification. *(This entry superseded the original open-problem text on 2026-07-26; the original statement and its literature placement are preserved in the outreach note and notes/LITSEARCH-2026-07-26-T1-T2.md.)*
