...
### 6.8 ADDENDUM (2026-07-26, later same day) — L-MIRROR settled: Box 4a RESTORED in re-scoped form

The open lemma L-MIRROR registered in 6.3 was proved in entry 6 (phases/phase105-entry6-joint-arena.md, refereed 2026-07-26; report in reviews/REPORTS-phase105-entry6-referee-2026-07-26.md). In the E2 plateau class — the class on which D₁₂ is *defined* — the mirrored representative is s₁∘r₁ = Q₁ − s₁, σ kills the constant, and **D₁₂(mirror-1, 2) = −D₁₂ exactly, in every configuration [refereed]**. Box 4a is restored with its domain made explicit. The winding-profile counterexamples of the 6.3 referee pass were adjudicated (and independently confirmed) to lie outside the invariant's domain; the winding-implementer mutual phase itself is reclassified as ill-posed pending an intrinsic charged-sector definition (open question Q-WIND). Bonus refereed corollary: mirroring defect *2* across r₁ leaves D₁₂ unchanged — one-sided flip on the mirrored side only, composing consistently with the both-mirrored flip (Box 4b).
