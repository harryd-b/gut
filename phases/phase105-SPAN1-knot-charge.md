...
## Amendment (2026-07-26) — KNOT-Q superseded by KNOT-Q v2 after the phase-107 campaign

*The phase-107 round-1 campaign (phases/phase107-KNOTQ-bridge.md, refereed 2026-07-26, report in reviews/REPORTS-phase107-KNOTQ-referee-2026-07-26.md) tested the §4 registration against contact topology. Outcome, with full disclosure: the original kill never fired — it HUNG (the literal decoration "Lutz twist along a transverse knot" always destroys the tight carrier, so the quantization was undefined, not zero-valued) — but the registration's mechanisms were refuted: the Bennequin-budget mechanism for (a) is a category error [refereed]; the trefoil-minimality clause (b) is numerology from the wrong carrier (Ghys's modular correspondence) [refereed]; the Arf clause (c) is ill-typed (Arf undefined for most charged classes; the statistics parity is torsion parity, a twist invariant) [refereed]. Clause (a)'s CONCLUSION survives under a corrected, forced mechanism (locality + tightness). Per referee order, the conjecture is re-registered:*

> **KNOT-Q v2 [conjecture; registered 2026-07-26; supersedes the 2026-07-24 registration; kill attached].** In the tight carrier (T¹Σ_g, ξ_can), g ≥ 2, the tightness-compatible quantized twist data — within the operation class {knot-attached Lutz twists, torus torsion insertions}, the proven scope [C-K1] — is exactly Giroux torsion n ∈ ℤ₊ on vertical tori over essential SIMPLE closed geodesics. The conjecture: the (to-be-defined) bulk-to-boundary bridge assigns to torsion n over γ a boundary defect of charge Q = e·n anchored at fix(γ), with e ≠ 0 (the holonomy principle), so that:
> (a) [conclusion retained; mechanism corrected] knots with contractible projection — in particular null-homologous knots of Seifert genus 0 — carry zero coupling-visible charge, by the refereed locality theorem plus tightness;
> (b′) [replaces (b)] Q ≠ 0 is realizable exactly over essential simple closed geodesics, at every n ≥ 1 — no knot-class minimality is conjectured;
> (s) [new, the discriminating prediction] charge is realizable ONLY over SIMPLE closed geodesics — the boundary theory's locality theorem permits any hyperbolic class, the bulk permits only simple ones; if boundary charge over a non-simple class is ever forced by the bridge, v2 is wrong in an informative direction.
> Struck as refuted: trefoil minimality; Arf ↦ statistics. Sign of Q: convention (orientation of Σ_g + anchor labeling), not conjectured.
>
> **Kill condition (v2, armed):** if the bridge, once defined without fitting (the definition must fix e before evaluating any clause), forces e = 0 (all charges vanish — the second disjunct re-registered over essential-simple-curve classes), or assigns nonzero coupling-visible charge to a contractible-projection class, KNOT-Q v2 is dead. The bridge-definition task — the boundary-net action of one Giroux-torsion insertion along one vertical torus — is the named F-GAP object [C-K2: a modeling task, not a computation, until defined].
>
> *Disclosure: this re-registration narrows the conjecture after two clauses were refuted. It is referee-ordered re-scoping with the refutations preserved above, not a fitting escape: the original kill was never dodged (it hung on an undefined quantization), and v2's kill is armed against the same failure modes in the corrected domain.*

*Addendum (2026-07-26, later): after the round-2 bridge derivation and referee pass (phases/phase107-KNOTQ-round2-bridge-definition.md), the operator adopted the bridge postulates as convention **C-BRIDGE** (see the round-2 amendment A.6, with mandatory disclosure): Q = ±n, D = n²ℓ/2π. Clauses (a)/(b′) of v2 are henceforth verified conditional on C-BRIDGE; the kill remains armed against any forced construction; clause (s) (simplicity) remains the open discriminating prediction.*
